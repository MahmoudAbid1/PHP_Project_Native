        ...
        <div class="container" id="container">
            <div class="form-container sign-up-container">
                <form action="#" method="post">
                    ...
                    <input type="password" name="password" autofocus placeholder="password" value="<?php echo htmlspecialchars($password) ?>" onkeyup="validatePassword()">
                    <span id="password-validation" class="error-message"></span>
                    <input type="text" name="phone_number" placeholder="Phone Number" onkeyup="validatePhoneNumber()">
...
function validatePhoneNumber() {
  ...
  if (!regex.test(phoneNumber)) {
    document.querySelector('#phone-validation').innerHTML = "The phone number must contain only numbers";
  } else {
...
function validatePassword() {
  ...
  var regex = /^(?=.*[0-9])(?=.*[*/\-_])[a-zA-Z0-9*\/\-_]+$/;
  var errorMessage = document.querySelector('#password-validation');
  if (!regex.test(password)) {
    errorMessage.innerHTML = "The password must contain at least one number and one character";
